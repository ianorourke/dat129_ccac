...
def scale_twice(dictionary):
    '''Scales the icon by 2x.'''
    for key in dictionary:
        for vert in range(0,2):
            for item in dictionary[key]:
                for digit in item:
                    if '0' in digit:
                        item = item.replace(digit,' ' * 2)
                    else:
                        item = item.replace(digit,'X' * 2)
            print(item)
            
def scale_thrice(dictionary):
    '''Scales the icon by 3x.'''
    for key in dictionary:
        for vert in range(0,3):
            for item in dictionary[key]:
                for digit in item:
                    if '0' in digit:
                        item = item.replace(digit,' ' * 3)
                    else:
                        item = item.replace(digit,'X' * 3)
            print(item)
...
count as a 1) for each row until all 10 are complete! Let's begin!''')
    ...
    menu_list = ['See Original','Reverse','Invert','Scale x 2','Scale x 3','End Program']
    ...
    while option_wheel != False:
        ...
        # Option for viewing the icon inverted.
            ...
            print('\nAny other transformations you would like to see?')
            print('')
        # Option for viewing the icon scaled x 2.
        elif option_input == '4':
            print('\nNow let\'s see how this looks scaled at 2x!')
            scale_twice(icon_dict)
            print('\nAny other transformations you would like to see?')
            print('')
        # Option for viewing the icon scaled x 3.
        elif option_input == '5':
            print('\nNow let\'s see how this looks scaled at 3x!')
            scale_thrice(icon_dict)
            print('\nAny other transformations you would like to see?')
            print('')
        # Ends the program.
        elif option_input == '6':
